...
# multidimensional Numpy int arrays
chunksize = 3
if rank == SCATTER_ROOT:
    ...
    scatter_data = None
my_data = world.scatter(scatter_data, root=SCATTER_ROOT)
assert numpy.alltrue(my_data == numpy.arange(rank*chunksize*2*5,(rank+1)*chunksize*2*5,dtype=numpy.float).reshape(2,chunksize,5) )

# 4D Numpy float array
if rank == SCATTER_ROOT:    
    scatter_data = numpy.arange(size*chunksize*12,dtype=numpy.float).reshape(size*3,chunksize,2,2)
else:
    scatter_data = None
